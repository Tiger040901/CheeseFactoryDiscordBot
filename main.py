...
async def verify(interaction:discord.Interaction, ign:str):
    ...
    if str(interaction.user) == discord_from_hypixel and 1291109419576197263 not in role_ids:
      await interaction.response.send_message("VERIFIED")
      guild = interaction.guild
      role = guild.get_role(1291109419576197263)
      await interaction.user.add_roles(role)
      
    else:
      await interaction.response.send_message("ERROR YOU NIGGA")
...
#    #verified_role = guild.get_role(1291109419576197263)
#    verified_role = guild.get_role(1291109419576197263)
#    member_with_low_level = []
#    #CATA ROLES
#    #cata24_27role = guild.get_role(1259296048397160549)
#    #cata28_31role = guild.get_role(1259296083549487147)
#    #cata32_35role = guild.get_role(1259296123755958322)
#    #cata36_39role = guild.get_role(1259296157192945704)
#    #cata40_44role = guild.get_role(1260192327352451154)
#    #cata45_49role = guild.get_role(1267655995656503327)
#    #cata50role = guild.get_role(1267656055404626062)
#
#    prova1 = guild.get_role(1292192691827900426)
#    prova2 = guild.get_role(1292192746991390750)
#
#
#
#    # Exclude bots from the member list, keeping the member objects
#    member_list = [member for member in members if not member.bot]
#    
...
#               if dungeonLevelInTheEnd < 24:
#                  member_with_low_level.append(member.name)
#               #elif dungeonLevelInTheEnd >= 24 and dungeonLevelInTheEnd < 28:
#               #   member.add_roles(cata24_27role)
#               #elif dungeonLevelInTheEnd >= 28 and dungeonLevelInTheEnd < 32:
#               #   member.add_roles(cata28_31role)
#               #elif dungeonLevelInTheEnd >= 32 and dungeonLevelInTheEnd < 36:
#               #   member.add_roles(cata32_35role)
#               #elif dungeonLevelInTheEnd >= 36 and dungeonLevelInTheEnd < 40:
#               #   member.add_roles(cata36_39role)
#               #elif dungeonLevelInTheEnd >= 40 and dungeonLevelInTheEnd < 45:
#               #   member.add_roles(cata40_44role)
#               #elif dungeonLevelInTheEnd >= 45 and dungeonLevelInTheEnd < 50:
#               #   member.add_roles(cata45_49role)
#               #elif dungeonLevelInTheEnd >= 50:
#               #   member.add_roles(cata50role)
#               elif dungeonLevelInTheEnd >= 30 and dungeonLevelInTheEnd < 36:
#                  member.add_roles(prova1)
#               elif dungeonLevelInTheEnd >= 36 and dungeonLevelInTheEnd < 41:
#                  member.add_roles(prova2)
#               await ctx.send(f"{member.name} has a dungeon level of {dungeonLevelInTheEnd}")
#
#              
#            except Exception as e:
#               print(e)
...
async def list_members(ctx):
    ...
    for member in member_list:
        await ctx.send(f"Processing {member.name}")
        
        if verified_role in member.roles and prova1 not in member.roles and prova2 not in member.roles:
            time.sleep(10)
            # Use member.nick if it's available, otherwise fall back to member.name
            name = member.nick if member.nick else member.name

            try:
                uuid = GetUUID(name)
                if not uuid:
                    await ctx.send(f"Could not fetch UUID for {name}, skipping.")
                    continue

                data = GetHypixelApiData(uuid)
                if not data or "profiles" not in data or not data["profiles"]:
                    await ctx.send(f"No SkyBlock profiles found for {name}, skipping.")
                    continue

                catacombs_xp_required = [
                    ...
                    9200000, 12000000, 15000000, 19000000, 24000000, 30000000, 38000000, 
                ...
                for profile in data["profiles"]:
                    try:
                        catacombs_xp = profile["members"][uuid]["dungeons"]["dungeon_types"]["catacombs"]["experience"]
                        dungeonLevels.append(catacombs_xp)
                    except KeyError:
                        continue

                if not dungeonLevels:
                    await ctx.send(f"No Catacombs data found for {name}, skipping.")
                    continue

                for dungeon_xp in dungeonLevels:
                    cumulative_xp = 0
                    for i in range(50):
                        cumulative_xp += catacombs_xp_required[i]
                        if cumulative_xp > dungeon_xp:
                            dungeonLevelsFinal.append(i)
                            break
                ...
                if dungeonLevelInTheEnd < 24:
                    ...
                    await member.add_roles(prova1)
                elif dungeonLevelInTheEnd >= 36 and dungeonLevelInTheEnd < 41:
                    await member.add_roles(prova2)
                ...
                continue
    await ctx.send("FINISHED processing member list.")
